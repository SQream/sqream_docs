.. _acceleration_studio_version_5.4.3:
...
:ref:`Back to Navigating Studio’s Main Features<top1_5.4.3>`

.. _studio_editor_5.4.3:
...
.. list-table::
   ...
   * - :ref:`Execution Tree<execution_tree_5.4.3>`
   ...
   * - :ref:`Save results to clipboard<save_results_to_clipboard_5.4.3>`
     - Lets you save your search results to the clipboard to paste into another text editor.
   * - :ref:`Save results to local file<save_results_to_local_file_5.4.3>`
...
Clicking **Execution Details View** displays the **Execution Tree**, which is a chronological tree of processes that occurred to execute your query. The purpose of the Execution Tree is to analyze all aspects of your query for troubleshooting and optimization purposes, such as resolving queries with an exceptionally long runtime. From this screen you can scroll in, out, and around the execution tree to analyze all aspects of your query. You can navigate around the execution tree by dragging or by using the **high level nagivation pane** in the bottom right corner. You can also search for query data by pressing **Ctrl+F** for typing text in the search field in the top right corner. Pressing **Enter** takes you directly to the node you've searched for.

The execution tree displays the same information as shown in the plain view in tree format. Note that the **Execution Details View** button is enabled only when a query takes longer than five seconds.

The Execution Tree tracks each phase of your query in real time as a vertical string of nodes moving in an upward direction. Each node refers to an operation that occurred on the GPU or CPU. When a phase is completed, the next string begins to its right in the same upward direction until the entire query is complete. Joins are displayed as two parallel strings merged together in a node called **Join**. The nodes are connected by a line indicating the number of rows passed from one node to the next. The Execution Tree tracks each phase of your query in real time starting with the bottom node in each string and moving up. When a phase is completed, it moves down and right to the next string and tracks it in the same upward direction. Each node refers to an operation that occurred on the GPU or CPU.

Each node displays a number displaying its **node ID**, its **type**, **table name** (if relevant), **status**, and **runtime in milliseconds**. The nodes are color-coded for easy identification. Green nodes indicate **completed nodes**, yellow indicates **nodes in progress**, and red indicates **slowest nodes**, typically joins.

Completed Queries

When a query is complete the following statistical information is displayed in the top left corner:

* **Query Statistics**:

    * Elapsed - the total time taken for the query to complete.
    * Result rows - the amount of rows fetched.
    * Running nodes completion - completed running nodes?
    * Total query completion - the amount of the total execution tree that was executed (nodes marked green).
	
* **Slowest Nodes** information is displayed in the top right corner. Clicking the slowest node centers automatically on that node in the execution tree.
...
* **Rows** - the number of produced rows passed to the next node
* **Chunks** - number of produced chunks
* **Average rows per chunk** - number of average rows per chunk
* **Table** (for **ReadTable** and joins only) - the table name
* **Write** (for joins only) - the total date size written to the disk
* **Read** (for **ReadTable** and joins only) - the total data size read from the disk

Note that you can scroll the Node Statistics table. When a query is complete, you can export the query results. You can also download the execution plan table in .csv format by clicking the download arrow in the upper-right corner.

For more information about exporting query results, see .
...
For more information, see **Execution Details View** below.
...
.. _configuration_5.4.3:

.. _configuration_1_5.4.3:
...
:ref:`Back to Configuring Your Instance of SQream<configuration_1_5.4.3>`
