...
As of June 2024, CentOS Linux 7.x will reach its End of Life. This announcement provides a one-year advance notice for our users to plan for this change. Due to the fact that CentOS will no longer support production deployments, we recommend users to explore migration to the Red Hat Enterprise Linux (RHEL) commercial Linux distribution developed by Red Hat.
