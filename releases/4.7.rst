...
Flag: ``ldapGetAttributeList`` - Declare in a list format which LDAP attributes are required to be seen for each role in MD.

Utility function: ``SELECT ldap_get_attr()`` - Shows required attributes mentioned in ``ldapGetAttributeList`` per MD role.
