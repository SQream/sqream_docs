...
► A new :ref:`Health-Check Monitor<select_health_check_monitoring>` utility command that allows administrators to monitor the database health

► A new :ref:`Query Timeout Per Service<>` cluster flag that is designed to identify queries that are stuck, having exceeded the specified time limit. Once the flag value is reached, the query automatically stops.
